...
export const registerUser = async (userData: typeof registerSchema._type): Promise<IUser> => {
    try {
        ...
        if (existingUser && existingUser.isDeleted) {
            ...
            existingUser.verificationTokenExpires = new Date(Date.now() + VERIFICATION_TOKEN_LIFETIME);
            await existingUser.save();

            await sendAccountReactivationEmail(existingUser.email, rawToken, existingUser._id.toString());

            throw new ConflictError("Tu cuenta estaba eliminada. Te enviamos un correo para reactivarla.");
        }
        ...
        user.verificationTokenExpires = new Date(Date.now() + VERIFICATION_TOKEN_LIFETIME);

        await user.save();
        await sendVerificationEmail(user.email, rawVerificationToken, user._id.toString());
        ...
        throw new InternalServerError('Ocurrió un error inesperado durante el registro.');
    }
};

export const loginUser = async (credentials: typeof loginSchema._type): Promise<{ user: IUser; token: string }> => {
    ...
    if (user.isDeleted) {
        ...
        await sendAccountReactivationEmail(user.email, rawToken, user._id.toString());

        throw new UnauthorizedError("Tu cuenta fue eliminada. Te enviamos un correo para reactivarla.");
    }

    if (!user.isVerified) {
        throw new UnauthorizedError('Por favor, verifica tu correo electrónico para iniciar sesión.');
    }
...
export const forgotPassword = async (email: string): Promise<string> => {
    ...
    user.passwordResetTokenExpires = new Date(Date.now() + VERIFICATION_TOKEN_LIFETIME);
    await user.save();
